...
function fixCodeTabs() {
  ...
  var codeTabContents = Array.from(
    document.querySelectorAll('.tab-content.tab-panel-style')
  ).filter(function (tabContent) {
    return tabContent.querySelector(
      '*:scope > .tab-content-text > div.highlight:only-child, *:scope > .tab-content-text > pre:not(.mermaid).pre-code:only-child'
    );
  });
  ...
  codeTabContents.forEach(function (tabContent) {
    ...
    var tabButton = tabPanel.querySelector(
      '.tab-nav-button.tab-panel-style[data-tab-item="' + tabId + '"]'
    );
...
function switchTab(tabGroup, tabId) {
  ...
  if (isButtonEvent) {
    ...
    if (window.localStorage) {
      var selectionsJSON = window.localStorage.getItem(
        window.relearn.absBaseUri + '/tab-selections'
      );
      if (selectionsJSON) {
      ...
      postRenderCallback: function (id) {
        ...
        svgs.each(function () {
          ...
          parent.style.maxWidth =
            parent.style.maxWidth || 'calc( ' + this.getAttribute('width') + 'px + 1rem )';
          var svg = d3.select(this);
          svg.html('<g>' + svg.html() + '</g>');
          var inner = svg.select('*:scope > g');
          parent.insertAdjacentHTML(
            'beforeend',
            '<span class="svg-reset-button" title="' +
              window.T_Reset_view +
          ...
          button.addEventListener('click', function (event) {
            ...
            this.classList.add(
              'tooltipped',
              'tooltipped-' + (doBeside ? 'w' : 's' + (isImageRtl ? 'e' : 'w'))
            );
          });
...
function initOpenapi(update, attrs) {
  ...
  function renderOpenAPI(oc) {
    ...
    var buster = window.themeUseOpenapi.assetsBuster
      ? '?' + window.themeUseOpenapi.assetsBuster
      : '';
    var print = isPrint || attrs.isPrintPreview ? 'PRINT-' : '';
    var theme = print
      ? `${relBasePath}/css/theme-relearn-light${mod}.css${buster}`
      : document.querySelector('#R-variant-style').attributes.href.value;
          ...
          var options = {
            ...
            onComplete: function () {
              if (isPrint || attrs.isPrintPreview) {
                oi.contentWindow.document
                  .querySelectorAll('.model-container > .model-box > button[aria-expanded=false]')
          ...
          if (oc.dataset.openapiSpec) {
            try {
              ...
              try {
                ...
                console.error(
                  'OpenAPI: file "' +
                    oc.dataset.openapiUrl +
                    '" could not be parsed as JSON or YAML'
                );
              }
            }
          } else {
...
function initCodeClipboard() {
  function getCodeText(node) {
    ...
    Array.from(code.querySelectorAll('*:scope > span > span:first-child:not(:last-child)')).forEach(
      function (lineno) {
        lineno.remove();
      }
    );
  ...
  for (var i = 0; i < codeElements.length; i++) {
    ...
    if (!isFirstLineCell && (inPre || text.length > 5)) {
      ...
      if (inTable) {
        ...
        while (
          p != document &&
          (p.tagName.toLowerCase() != 'div' || !p.classList.contains('highlight'))
        ) {
          p = p.parentNode;
        }
        if (p == document) {
  ...
  clip.on('success', function (e) {
    e.clearSelection();
    var inPre =
      e.trigger.previousElementSibling &&
      e.trigger.previousElementSibling.tagName.toLowerCase() == 'pre';
    var isCodeRtl = !inPre ? isRtl : false;
    var doBeside =
      inPre ||
      (e.trigger.previousElementSibling &&
        e.trigger.previousElementSibling.tagName.toLowerCase() == 'table');
    e.trigger.setAttribute('aria-label', window.T_Copied_to_clipboard);
    e.trigger.classList.add(
      'tooltipped',
      'tooltipped-' + (doBeside ? 'w' : 's' + (isCodeRtl ? 'e' : 'w'))
    );
  });
    ...
    if (!event.shiftKey && !event.ctrlKey && event.altKey && !event.metaKey) {
      if (event.which == 38) {
        ...
        var target = isPrint
          ...
          : document.querySelector('.flex-block-wrapper');
  ...
  document.addEventListener('keydown', function (event) {
    if (!event.shiftKey && !event.ctrlKey && !event.altKey && !event.metaKey) {
      ...
      if (event.which == dir_key_end) {
        if (
          !scrollEnd &&
          +el.scrollLeft.toFixed() * dir_scroll + +el.clientWidth.toFixed() >=
            +el.scrollWidth.toFixed()
        ) {
          next && next.click();
        } else if (scrollEnd != -1) {
...
function toggleTopbarFlyoutEvent(event) {
  if (
    event.target.classList.contains('topbar-content') ||
...
function initSwipeHandler() {
  ...
  document
    .querySelector('#R-sidebar')
    .addEventListener('touchstart', handleStartX, { capture: false, passive: true });
  ...
  document
    .querySelector('#R-sidebar')
    .addEventListener('touchmove', handleMoveX, { capture: false, passive: true });
  document.querySelectorAll('#R-sidebar *').forEach(function (e) {
  ...
  document
    .querySelector('#R-sidebar')
    .addEventListener('touchend', handleEndX, { capture: false, passive: true });
  document.querySelectorAll('#R-sidebar *').forEach(function (e) {
...
function initHistory() {
  ...
  for (var item in sessionStorage) {
    if (
      item.substring(0, visitedItem.length) === visitedItem &&
...
function mark() {
  ...
  var bodyInnerLinks = document.querySelectorAll(
    '#R-body-inner a:not(.lightbox-link):not(.btn):not(.lightbox-back)'
  );
  ...
  for (var i = 0; i < markedElements.length; i++) {
    ...
    while (parent && parent.classList) {
      ...
      if (
        ...
        parent.parentNode &&
        parent.parentNode.tagName.toLowerCase() === 'ul' &&
        parent.parentNode.classList.contains('collapsible-menu')
      ) {
...
(function () {
  ...
  function moveAreaTopbarButtons(width) {
    topbar.querySelectorAll('.topbar-hidden .topbar-button').forEach(function (button) {
      ...
      var placeholder = button.parentNode.parentNode.querySelector(
        ':scope > .topbar-area .topbar-placeholder[data-action="hide"]'
      );
      placeholder.dataset.action = 'show';
      button.dataset.action = 'show';
    ...
    topbar.querySelectorAll('.topbar-area .topbar-button').forEach(function (button) {
      ...
      if (current_area != 'show' && origin_area != current_area) {
        ...
        var placeholder = topbar.querySelector(
          '.topbar-area[data-area="' +
            origin_area +
            '"] > .topbar-placeholder[data-action="' +
            current_area +
            '"]'
        );
        placeholder.dataset.action = 'show';
    ...
    Array.from(topbar.querySelectorAll('.topbar-area .topbar-button'))
      ...
      .forEach(function (button) {
        ...
        if (action == 'show') {
          ...
          // move to action area
          var dest = button.parentNode.parentNode.querySelector(
            '.topbar-area[data-area="' + action + '"] > *'
          );
          if (dest) {
            var placeholder = button.nextSibling;
            placeholder.dataset.action = action;
  ...
  function adjustEmptyTopbarContents() {
    var buttons = Array.from(
      document.querySelectorAll('.topbar-button > .topbar-content > .topbar-content-wrapper')
    );
    ...
    buttons.reverse().forEach(function (wrapper) {
      ...
      if (button) {
        ...
        button.style.display =
          isEmpty && button.dataset.contentEmpty == 'hide' ? 'none' : 'inline-block';
      }
    });
  }
